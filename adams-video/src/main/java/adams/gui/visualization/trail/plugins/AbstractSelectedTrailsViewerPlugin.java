...
/**
 * AbstractSelectedTrailsViewerPlugin.java
...
public abstract class AbstractSelectedTrailsViewerPlugin
  ...
  protected TrailPanel[] getAllPanels() {
    ...
    TrailViewerPanel owner;
    
    result = new ArrayList<TrailPanel>();
    
    owner = (TrailViewerPanel) GUIHelper.getParent(m_CurrentPanel, TrailViewerPanel.class);
    if (owner != null)
  ...
  /**
   * Creates the panel containing the list of all trails. Also updates
  ...
  protected JPanel createListPanel(final ApprovalDialog dialog) {
	...
	m_SelectedPanels = new TrailPanel[indices.length];
  ...
  protected String process() {
    ...
    if (result == null) {
      for (TrailPanel panel: m_SelectedPanels) {
