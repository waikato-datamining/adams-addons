...
/*
 * AbstractAudioFeatureGenerator.java
 ...
 */
public abstract class AbstractAudioFeatureGenerator<T extends AbstractAudioContainer>
  extends AbstractOptionHandler
  implements Comparable, CleanUpHandler, ShallowCopySupporter<AbstractAudioFeatureGenerator> {
   ...
   * <br><br>
   * Default implementation only checks whether container is null.
   *
   ...
   */
  public abstract HeaderDefinition createHeader(T cont);
   ...
   * 
   * @param cont	the container container
  ...
  public List<Object> postProcessRow(T cont, List<Object> data) {
    ...
    // notes
    for (i = 0; i < m_Notes.length; i++) {
      valueStr = cont.getNotes().getPrefixSubset(m_Notes[i].getValue()).toString();
      data.add(valueStr);
    }
  ...
  public List<Object>[] postProcessRows(T cont, List<Object>[] data) {
    ...
    for (i = 0; i < result.length; i++)
      result[i] = postProcessRow(cont, data[i]);
    
    return result;
  }
  
  /**
   * Process the given container. This method will also create the header if
   * necessary.
   *
   * @param cont	the container to process
  ...
  public Object[] generate(T cont) {
    ...
    data   = generateRows(cont);
    data   = postProcessRows(cont, data);
  ...
  public AbstractAudioFeatureGenerator shallowCopy(boolean expand) {
    return (AbstractAudioFeatureGenerator) OptionUtils.shallowCopy(this, expand);
  }
