...
\begin{tight_itemize}
  ...
  \item \textit{NewAudioTrail} -- creates an empty audio trail data structure.
\end{tight_itemize}

\noindent The following transformers are available:
\begin{tight_itemize}
  \item \textit{AddAudioTrailStep} -- adds a step to an audio trail data structure.
  ...
  \item \textit{AudioTrailFileReader} -- reads an audio trail from disk.
  \item \textit{AudioTrailFileWriter} -- writes an audio trail to disk.
  \item \textit{AudioTrailFilter} -- applies a filter to an audio trail.
