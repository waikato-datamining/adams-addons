...
/*
 * AbstractHeatmapFeatureGenerator.java
...
public abstract class AbstractHeatmapFeatureGenerator
   ...
   */
  public abstract HeaderDefinition createHeader(Heatmap map);
   ...
   * 
   * @param map		the heatmap container
  ...
  public List<Object> postProcessRow(Heatmap map, List<Object> data) {
    ...
    // notes
    for (i = 0; i < m_Notes.length; i++) {
      valueStr = map.getNotes().getPrefixSubset(m_Notes[i].getValue()).toString();
      data.add(valueStr);
    }
  ...
  public List<Object>[] postProcessRows(Heatmap map, List<Object>[] data) {
    ...
    for (i = 0; i < result.length; i++)
      result[i] = postProcessRow(map, data[i]);
    
    return result;
  }
  
  /**
   * Process the given heatmap. This method will also create the header if
   * necessary.
   *
   * @param map		the heatmap to process
   * @return		the generated array
   * @see		#createHeader(Heatmap)
   */
  public Object[] generate(Heatmap map) {
    ...
    data   = generateRows(map);
    data   = postProcessRows(map, data);
  ...
  public AbstractHeatmapFeatureGenerator shallowCopy(boolean expand) {
    return (AbstractHeatmapFeatureGenerator) OptionUtils.shallowCopy(this, expand);
  }
