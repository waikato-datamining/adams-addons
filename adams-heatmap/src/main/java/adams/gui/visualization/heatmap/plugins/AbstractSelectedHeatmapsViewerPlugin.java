...
/**
 * AbstractSelectedHeatmapsViewerPlugin.java
...
public abstract class AbstractSelectedHeatmapsViewerPlugin
  ...
  protected HeatmapPanel[] getAllPanels() {
    ...
    HeatmapViewerPanel owner;
    
    result = new ArrayList<HeatmapPanel>();
    
    owner = (HeatmapViewerPanel) GUIHelper.getParent(m_CurrentPanel, HeatmapViewerPanel.class);
    if (owner != null)
  ...
  /**
   * Creates the panel containing the list of all heatmaps. Also updates
  ...
  protected JPanel createListPanel(final ApprovalDialog dialog) {
	...
	m_SelectedPanels = new HeatmapPanel[indices.length];
  ...
  protected String process() {
    ...
    if (result == null) {
      for (HeatmapPanel panel: m_SelectedPanels) {
