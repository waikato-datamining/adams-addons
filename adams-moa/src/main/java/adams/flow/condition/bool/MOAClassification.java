...
public class MOAClassification
  ...
  protected AbstractClassifier m_Model;
  ...
  protected String setUpModel(Actor owner) {
	...
	obj    = CallableActorHelper.getSetup(AbstractClassifier.class, m_ModelActor, owner, errors);
	if (obj == null) {
	  if (!errors.isEmpty())
	...
	getLogger().log(Level.SEVERE, msg, e);
      ...
      // load model
      try {
	m_Model = (AbstractClassifier) SerializationHelper.read(m_ModelFile.getAbsolutePath());
      }
      catch (Exception e) {
	m_Model = null;
