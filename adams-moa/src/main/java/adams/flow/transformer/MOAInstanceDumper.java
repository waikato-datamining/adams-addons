...
/**
 <!-- globalinfo-start -->
 * Dumps MOA Instance objects into an ARFF file. If the headers change and the header-check is enabled, then a new file will be used.<br>
 ...
 */
public class MOAInstanceDumper
  extends AbstractTransformer
  implements BufferSupporter, FlushSupporter  {

  /** for serialization. */
  private static final long serialVersionUID = -4766458896935070355L;
  ...
  public String globalInfo() {
    return
      "Dumps MOA Instance objects into an ARFF file. If the headers "
        + "change and the header-check is enabled, then a new file will be used.\n"
        + "The actor can also turn double arrays into MOA Instance objects "
   ...
   *
   * @return		<!-- flow-accepts-start -->com.yahoo.labs.samoa.instances.Instance.class, double[].class<!-- flow-accepts-end -->
   */
  ...
  protected File createFilename(Instances header) {
    ...
    if (m_UseRelationNameAsFilename) {
      ...
      result =   file.getParent()
        + File.separator
        + FileUtils.createFilename(header.getRelationName(), "_");
    }
    else {
  ...
  protected String doExecute() {
    ...
    if (m_Header == null) {
      ...
      if (m_CheckHeader) {
        if (!MOAHelper.equalHeaders(m_Header, newHeader)) {
          m_Counter++;
          m_Header = new Instances(newHeader, 0);
          append  = false;
        }
