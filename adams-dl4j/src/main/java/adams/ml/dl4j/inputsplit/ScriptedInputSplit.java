...
public class ScriptedInputSplit
  ...
  protected String check() {
    ...
    if (result == null)
      m_InputSplitObject = (InputSplit) m_ScriptObject;
  ...
  public void destroy() {
    ...
    m_InputSplitObject = null;
  }

  /**
   * Returns the input split. Raises an {@link IllegalStateException} if not
   * model object loaded.
   *
   * @return		the input splot
   */
  protected synchronized InputSplit getInputSplit() {
    if (m_InputSplitObject != null)
      return m_InputSplitObject;
    else
