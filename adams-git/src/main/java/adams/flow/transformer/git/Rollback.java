...
public class Rollback
  ...
  public String globalInfo() {
    return "Performs 'git checkout' on the incoming file(s), essentially rolling back any changes.";
  }
  ...
  protected boolean doCanExecute(Object input, MessageCollection errors) {
    ...
    files = toFiles(input);
    return (files.length > 0) && m_GitOperation.canRollback(files, errors);
  }
  ...
  protected Object doExecute(Object input, MessageCollection errors) {
    ...
    files = toFiles(input);
    return m_GitOperation.rollback(files, errors);
  }
}
